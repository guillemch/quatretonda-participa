  ...
  <section class="booth" id="boothView" :aria-label="$t('booth.label')">
    <transition :name="transitionName" mode="out-in">
      <router-view
        ...
        :receipt="receipt"
        @doneSelecting="doneSelectingQuestion" />
    </transition>
  ...
  export default {
    ...
    methods: {
      ...
      checkboxOptions (option) {
        ...
        const questionIndex = selected.findIndex((q) => q.id === option.question_id);
        const optionIndex = selected[questionIndex].options.findIndex((o) => o.id === option.id);
      ...
      doneSelectingQuestion (id, doneSelecting) {
        if (doneSelecting) {
          this.doneSelectingQuestions[id] = true
        } else {
          this.doneSelectingQuestions.splice(id, 1)
        }

        const allAnswered = Object.keys(this.doneSelectingQuestions).length === this.ballot.questions.length
        if (allAnswered) this.scrollToID()
      },
